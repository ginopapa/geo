...
if ($method=="POST"){
	...
	if ($table=="bundles"){
		...
		$sql="INSERT INTO bundles (id, published) VALUES (".substr($now, 4).", 'yes')";
		mysqli_query($con,$sql);
		$sql="INSERT INTO paths (id, bundleID, published) VALUES (".substr($now, 3).", ".substr($now, 4).", 'yes')";
		mysqli_query($con,$sql);
		$sql="INSERT INTO places (id, pathID, published) VALUES (".substr($now, 2).", ".substr($now, 3).", 'yes')";
		mysqli_query($con,$sql);
		$polyline='[{"lat": 0, "lng": 0}]';
		$sql="INSERT INTO polyline (pathID, published, polyline) VALUES (".substr($now, 3).", 'yes', '".$polyline."')";
		mysqli_query($con,$sql);
		$sql="INSERT INTO media (id, placeID, published) VALUES (".substr($now, 1).", ".substr($now, 2).", 'yes')";
		mysqli_query($con,$sql);
		echo substr($now, 4);
	}
	...
	switch (count($request_parts)) {
		...
		case 1: //Contact all db, admin client uses this one
			if (strtoupper($request_parts[0])=="BUNDLE"){
				$sqlbundle="SELECT * FROM bundles WHERE published = 'yes' ORDER BY id ASC";
			} else {
				$id=$request_parts[0];
				$sqlbundle="SELECT * FROM bundles WHERE published = 'yes' AND id=".$id." ORDER BY id ASC";
			}
			$sqlpaths="SELECT * FROM paths WHERE published = 'yes'";
			$sqlplaces="SELECT * FROM places WHERE published = 'yes'";
			$sqlpolylines="SELECT * FROM polyline WHERE published = 'yes'";
			$sqlmedia="SELECT * FROM media WHERE published = 'yes'";
		break;
